...
3. **Peers de Descoberta**  
   Participam dos mecanismos de identificação e localização de outros nós. No trabalho proposto, esse papel inicial é exercido pelo **servidor Rendezvous**, responsável por registrar e listar peers ativos. Posteriormente, a descoberta pode ser descentralizada via mensagens de controle como `WHO_HAS`.

4. **Peers de Armazenamento e Cache**  
   Mantêm dados temporários ou permanentes que podem ser redistribuídos quando solicitados. Esse papel é comum em redes de compartilhamento de arquivos, mas pode ser adaptado em sistemas de mensageria para suportar histórico ou retransmissão.

5. **Peers de Observabilidade**  
